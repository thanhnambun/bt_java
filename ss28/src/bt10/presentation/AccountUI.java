...
import bt10.business.service.account.AccountService;
import bt10.business.service.account.AccountServiceImp;
...
public class AccountUI {
    ...
    public static void displayAccountMenu(Scanner scanner) {
        ...
        do {
            ...
            switch (choice) {
                case 1:
                    listAccountMenu(accountService);
                    break;
                case 2:
                    addAccountMenu(scanner,accountService);
                    break;
                case 3:
                    updateAccountMenu(scanner,accountService);
                    break;
                case 4:
                    deleteAccountMenu(scanner,accountService);
                    break;
                case 5:
    ...
    public static void listAccountMenu(AccountService accountService) {
        accountService.getAll();
    }

    public static void addAccountMenu(Scanner sc,AccountService accountService) {
        Account acc = new Account();
        acc.inputData(sc);
        accountService.save(acc);
    }
    public static void updateAccountMenu(Scanner sc,AccountService accountService) {
        System.out.println("vui lòng nhập id");
        int id = Integer.parseInt(sc.nextLine());
        accountService.update(id);
    }

    public static void deleteAccountMenu(Scanner sc,AccountService accountService) {
        System.out.println("vui lòng nhập id");
        int id = Integer.parseInt(sc.nextLine());
    ...
    public static void checkbalance(Scanner sc, AccountService accountService){
        System.out.println("vui lòng nhập id");
        int id = Integer.parseInt(sc.nextLine());
        accountService.getBalanceByAccountId(id);
    }
}
